...
#property strict
#property description "Buy-only EMA cross EA with EMA 200 trend filter for any symbol and timeframe."
...
ENUM_TIMEFRAMES g_strategy_timeframe = PERIOD_H4;
...
bool ReadClosedBarClose(double &closed_bar_close)
{
   ...
   if(CopyClose(_Symbol, g_strategy_timeframe, 1, 1, closes) != 1)
      return false;
...
int OnInit()
{
   ...
   g_ema20_handle = iMA(_Symbol, g_strategy_timeframe, InpFastEmaPeriod, 0, MODE_EMA, PRICE_CLOSE);
   g_ema50_handle = iMA(_Symbol, g_strategy_timeframe, InpSlowEmaPeriod, 0, MODE_EMA, PRICE_CLOSE);
   g_ema200_handle = iMA(_Symbol, g_strategy_timeframe, FILTER_EMA_PERIOD, 0, MODE_EMA, PRICE_CLOSE);

...
#property strict
#property description "Buy-only EMA cross EA with EMA 200 trend filter for any symbol on H4."
...
const ulong STRATEGY_MAGIC = 26022501;
...
bool IsNewStrategyBar(datetime &bar_time)
{
   ...
   if(CopyTime(_Symbol, STRATEGY_TIMEFRAME, 0, 2, bars) != 2)
      return false;
...
bool ReadClosedBarClose(double &closed_bar_close)
{
   ...
   if(CopyClose(_Symbol, STRATEGY_TIMEFRAME, 1, 1, closes) != 1)
      return false;
...
bool FindOpenBuyPosition(ulong &ticket)
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(symbol == _Symbol && magic == STRATEGY_MAGIC && type == POSITION_TYPE_BUY)
      {
         ticket = pos_ticket;
         return true;
      }
...
int OnInit()
{
   ...
   {
      PrintFormat("Attach this EA to %s timeframe.", EnumToString(STRATEGY_TIMEFRAME));
      return INIT_PARAMETERS_INCORRECT;
   }

   if(InpFastEmaPeriod <= 0 || InpSlowEmaPeriod <= 0 || FILTER_EMA_PERIOD <= 0)
   {
   ...
   g_ema20_handle = iMA(_Symbol, STRATEGY_TIMEFRAME, InpFastEmaPeriod, 0, MODE_EMA, PRICE_CLOSE);
   g_ema50_handle = iMA(_Symbol, STRATEGY_TIMEFRAME, InpSlowEmaPeriod, 0, MODE_EMA, PRICE_CLOSE);
   g_ema200_handle = iMA(_Symbol, STRATEGY_TIMEFRAME, FILTER_EMA_PERIOD, 0, MODE_EMA, PRICE_CLOSE);
